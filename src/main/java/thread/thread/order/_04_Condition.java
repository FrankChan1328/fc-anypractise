 ...
 * <p>Condition（条件变量）:通常与一个锁关联。需要在多个Contidion中共享一个锁时，可以传递一个Lock/RLock实例给构造方法，否则它将自己生成一个RLock实例。
 * <li>Condition中await()方法类似于Object类中的wait()方法。</li>
 * <li>Condition中await(long time,TimeUnit unit)方法类似于Object类中的wait(long time)方法。</li>
 * <li>Condition中signal()方法类似于Object类中的notify()方法。</li>
 * <li>Condition中signalAll()方法类似于Object类中的notifyAll()方法。</li>
 * 
 * <p>应用场景：Condition是一个多线程间协调通信的工具类，使得某个或者某些线程一起等待某个条件（Condition）,
 * 只有当该条件具备(signal 或者 signalAll方法被调用)时 ，
 * 这些等待线程才会被唤醒，从而重新争夺锁。
 */
public class _04_Condition {
