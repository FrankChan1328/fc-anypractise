...
public class PredictOldUtil {
    ...
    public static int getHitSeq(List<Lot> lots, int term, int eIndex) {
        ...
        NumProbability numPro = getOldPredictENextTerm(lots, term, eIndex).stream()
    ...
    public static void printOldE(List<Lot> totalLots, int term, int eIndex) {
        Lot lot = totalLots.stream().filter(it -> it.getTerm() == term).findFirst().get();
        printOldPredict(totalLots, lot.getE(eIndex), eIndex, term);
    }
    
    public static void printOldPredict(List<Lot> totalLots, int e0x, int index, int term) {
        NumProbability numPro = getOldPredictENextTerm(totalLots, term, index).stream().filter(it -> it.getNum() == e0x)
                .findFirst().get();
     ...
     * @return
     */
    public static List<NumProbability> getOldPredictENextTerm(List<Lot> totalLots, int term, int eIndex) {
        Lot lot = totalLots.stream().filter(it -> it.getTerm() == term).findFirst().get();
        NumProbability numPro = predictOldENextTerm(totalLots, term, eIndex).stream()
                .filter(it -> it.getNum() == lot.getE(eIndex)).findFirst().get();
        return numPro.getNextPros().stream().sorted(Comparator.comparing(NumProbability::getProbability).reversed())
                .collect(Collectors.toList());
    }
    
    public static List<NumProbability> predictOldENextTerm(List<Lot> totalLots, int term, int eIndex) {
            ...
            for (int k = 1; k <= 12; k++) {
                ...
                for (int j = 0; j < total; j++) {
                    ...
                    if (lot.getE(eIndex) == num) {
                        if (j < total - 2) {
                            if (lots.get(j + 2).getE(eIndex) == toCompare) {
                                count++;
                            }
                        }
                    }
